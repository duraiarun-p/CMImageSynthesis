...
def custom_loss_2_beta3D(y_true, y_pred):# SSIM
    ...
    for batchelei in range(batchsize[0]):
        ...
        for depthelei in range(depthsize[0]):
            ...
            ssimscoreele,_=ssTF.tfmsssim(y_true2, y_pred2, max_val,filter_size,filter_sigma)
            ssimscores.append(ssimscoreele)
    ssimscore=tf.reduce_mean(ssimscores)
    # ssimscore=ssTF.tfssim(y_true, y_pred, max_val,filter_size,filter_sigma)
    # loss=tf.math.subtract(1, ssimscore)# Will cause error when train_on_batch: run_eagerly=False
...
weightoutputpath='/home/arun/Documents/PyWSPrecision/Pyoutputs/cycleganweights/12032022/gamma/'
# imgshape=(512,512)
...
# batch_size=1
# epochs=1
cGAN=CycleGAN(mypath,weightoutputpath,epochs=1,batch_size=1,imgshape=(256,256,1),batch_set_size=1,saveweightflag=False)
# def run_tf(cGAN):
...
# data=loadprintoutgen(trainCT_path,trainCB_path,batch_size=5,batch_set_size=10)
