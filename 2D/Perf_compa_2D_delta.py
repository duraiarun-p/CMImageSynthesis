...
from CycleGAN_Archs_lib import CycleGAN_Delta, dataload, dataload_direct, perf_metrics
...
weightoutputpath=os.path.join(weightoutputpath1, 'delta_predicted_volume')
if not os.path.isdir(weightoutputpath):
    os.mkdir(weightoutputpath)
    
# cGAN=CycleGAN_Delta(mypath,weightoutputpath,epochs=40,save_epoch_frequency=2,batch_size=3,imgshape=(256,256,1),newshape=(256,256),batch_set_size=10,saveweightflag=True)
lastweightpath='/home/s1785969/RDS/PyWS/Pyoutputs/cycleganweights/CMImageSynthesis_Outputs/Gamma_Output/run0/weights'
cGAN=CycleGAN_Delta(mypath,weightoutputpath,lastweightpath,epochs=40,save_epoch_frequency=2,batch_size=3,imgshape=(256,256,1),newshape=(256,256),batch_set_size=10,saveweightflag=True,breakflag=False)
