...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 22 15:54:19 2022
...
#%%
def I2I_2D_CT(batch_CT,TestGenCT2CB,TestGenCB2CT):    
    batch_CT=tf.image.resize(batch_CT,[256,256])
    ...
    # batch_CT=tf.transpose(batch_CT,[1,2,0,3])
    ...
    batch_CT_P=np.transpose(batch_CT_P,(1,2,0))
    return batch_CT_P

def I2I_2D_CB(batch_CB,TestGenCT2CB,TestGenCB2CT):    
    batch_CB=tf.image.resize(batch_CB,[256,256])
    ...
    batch_CB=tf.transpose(batch_CB,[1,2,0,3])
...
def normalise_img_volume_1(CT):
    # CTsiz=CT.shape
    if np.min(CT)<0:
        CTs=CT+abs(np.min(CT))
    else:
        CTs=CT
    CTs=(CTs-np.min(CTs))/(np.max(CTs)-np.min(CTs))
    return CTs

def normalise_img_volume(CT):
    CTsiz=CT.shape
    CTs=np.zeros_like(CT)
    for zi in range(CTsiz[2]):
            if np.max(CT[:,:,zi])==0:
                image=np.zeros_like(CT[:,:,zi])
            else:
                image=CT[:,:,zi]
                image=image+abs(np.min(image))
                # This normalisation was used in the training so dont change it.
...
def I2Isynthesis_2D(CBCT_n,GenCB2CT):
        
    batch_CB = tf.image.resize(CBCT_n,[256,256])
    batch_CB = tf.transpose(batch_CB,perm=[2,0,1])
    batch_CB = tf.expand_dims(batch_CB, -1)
    
    batch_CT_P=GenCB2CT.predict(batch_CB)
