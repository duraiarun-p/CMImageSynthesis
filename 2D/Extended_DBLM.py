...
#%%
# def cus_corr2D(batch_CT,batch_CT_P):
#     N=tf.math.multiply((batch_CT-tf.math.reduce_mean(batch_CT)),(batch_CT_P-tf.math.reduce_mean(batch_CT_P)))
#     D=tf.math.multiply((tf.math.reduce_std(batch_CT)),(tf.math.reduce_std(batch_CT_P)))
#     NCC_score1=tf.math.reduce_mean(N/D)
#     return NCC_score1

# NCC_score1=cus_corr2D(batch_CT,batch_CT_P)
...
#%% Performance metrics # Cross correlation using scipy
def custom_corr2d(y_true, y_pred):
    batchsize=y_true.shape
    corr_max=[]
    for batchelei in range(batchsize[0]):
        y_pred1=y_pred[batchelei,:,:]
        y_true1=y_true[batchelei,:,:]
        corr = signal.correlate2d(y_true1, y_pred1, boundary='symm', mode='same')
        corr_max_ele = np.max(corr)
        corr_max.append(corr_max_ele)
    corr_val=tf.reduce_mean(corr_max)
    return corr_val
    
# CORR_score=custom_corr2d(batch_CT, batch_CT_P)
